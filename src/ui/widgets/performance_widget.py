# src/ui/widgets/performance_widget.py
...
from textual.widgets import Label, Button, DataTable, Static, Select
...
class TradeHistoryPanel(Static):
    ...
    def update_trades(self, trades_df: pd.DataFrame) -> None:
        ...
        # Process in chunks
        chunks = np.array_split(recent_trades, max(1, len(recent_trades) // self.CHUNK_SIZE))
        for chunk in chunks:
